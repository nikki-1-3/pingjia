...
if all_aspects:
    ...
    ax2.bar(x - width/2, pos_vals, width, label='好评提及', color='#4CAF50')
    ax2.bar(x + width/2, neg_vals, width, label='差评提及', color='#F44336')
    ax2.set_xticks(x)
    ax2.set_xticklabels(all_aspects, rotation=15)
    ax2.set_ylabel('提及次数')
    ax2.set_title('各属性在好评/差评中的提及次数对比')
    ax2.legend()
    plt.tight_layout()
    st.pyplot(fig2)
    ...
    ax3.plot(angles_r, pos_vals_r, 'o-', linewidth=2, label='好评', color='#4CAF50')
    ax3.fill(angles_r, pos_vals_r, alpha=0.25, color='#4CAF50')
    ax3.plot(angles_r, neg_vals_r, 'o-', linewidth=2, label='差评', color='#F44336')
    ax3.fill(angles_r, neg_vals_r, alpha=0.25, color='#F44336')
    ax3.set_xticks(angles)
    ax3.set_xticklabels(all_aspects)
    ax3.set_title('属性情感雷达图')
